...
bool RoomsPlugin::LoginRoomsParticipant(RakNet::RakString userName, SystemAddress roomsParticipantAddress, RakNetGUID guid, SystemAddress loginServerAddress)
{
	...
	bool objectExists;
	const unsigned int index = roomsParticipants.GetIndexFromKey(userName, &objectExists);
...
bool RoomsPlugin::LogoffRoomsParticipant(RakNetGUID guid, RakNet::RakString* userName) {
	userName = nullptr;

	for (unsigned int i = 0; i < roomsParticipants.Size(); i++) {
		if(roomsParticipants[i]->GetGUID() == guid) {
			RakString name = roomsParticipants[i]->GetName();
			userName = &name;
